...
#import "YJFImageLooper.h"

static NSInteger const radio = 3;

@interface YJFImageLooper ()<UIScrollViewDelegate>
{
...
@implementation YJFImageLooper

// 快速创建方法
+ (instancetype)imageRototarWithLoadImageBlock:(YJFLoadImageBlock)loadBlock{
    
    YJFImageLooper *imageLooper = [[YJFImageLooper alloc] init];
    imageLooper.loadBlock = loadBlock;
    return imageLooper;
}
...
-(void)setPicModels:(NSArray<id <YJFImageLooperProtocol>> *)picModels
{
    ...
    for (int i = 0; i< count; i++) {
        ...
        id<YJFImageLooperProtocol> picM = picModels[i % baseCount];
        // 1. 创建控件
        UIImageView *imageView = [[UIImageView alloc] init];
        imageView.tag = self.adPics.count;
        
        NSLog(@"%s--%zd",__func__,self.adPics.count);
...
- (void)jumpToLink:(UITapGestureRecognizer *)gester {
    ...
    id<YJFImageLooperProtocol> adM = self.picModels[tag];
...
- (void)caculateCurrentPage: (UIScrollView *)scrollView {
    ...
    if (self.picModels.count == 1) {
        _currentPage = 1;
        if ([self.delegate respondsToSelector:@selector(imageRototaDidSelectedPicModel:)]) {
            [self.delegate imageRototaDidSelectedPicModel:self.picModels[self.pageControl.currentPage]];
        }
        return;
    }
    // 确认中间区域
